...
int main() {
    ...
    while(sel_ret = select(max(tcp_sockid, udp_sockid)+1, &readfd, NULL, NULL, NULL)) {
        if(sel_ret < 0) {
            perror("select error");
            exit(EXIT_FAILURE);
        }
        ...
        if(FD_ISSET(tcp_sockid, &readfd)) {
            ...
            switch(fork()){
                    ...
                    case -1:
                        perror("[TCP] child creation failed");
                        exit(EXIT_FAILURE);
                    case 0:			/* CHILD */
            ...
            if(strcmp(de->d_name,".") && strcmp(de->d_name,"..")) {
            	...
            	if((file_fd = open(path1, O_RDONLY)) == -1) {
			        ...
			        while(read_size = read(file_fd, buffer, BUFFERSIZE)) {
                                ...
                                // if(read_size < BUFFERSIZE){                      // appending \0 to make it BUFFER_SIZE
                                //     while(read_size <= BUFFERSIZE){
                                //         buffer[read_size++] = '\0';
                                //     }
                                // }
